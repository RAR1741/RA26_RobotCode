...
/**
 * An Choreo specific {@code SendableChooser} that allows for the selection of
 * {@link AutoRoutine}s
 * at runtime via a <a
 * href=
 * "https://docs.wpilib.org/en/stable/docs/software/dashboards/index.html#dashboards">Dashboard</a>.
 *
 * <p>
 * This chooser takes a
 * <a href="https://en.wikipedia.org/wiki/Lazy_loading">lazy loading</a>
 * approach to {@link AutoRoutine}s, only generating the {@link AutoRoutine}
 * when it is selected.
 * This approach has the benefit of not loading all autos on startup, but also
 * not loading the auto
 * during auto start causing a delay.
 *
 * <p>
 * Once the {@link AutoChooser} is made you can add {@link AutoRoutine}s to it
 * using {@link
 * #addRoutine} or add {@link Command}s to it using {@link #addCmd}. Similar to
 * {@code
 ...
 *
 * <p>
 * You can set the Robot's autonomous command to the chooser's chosen auto
 * routine via <code>
...
public class AutoChooser implements Sendable {
  ...
  /**
   * Constructs a new {@link AutoChooser} with the given name for the do-nothing
   * default option.
   *
   ...
   *
   * <p>
   * This is done to load AutoRoutines when and only when they are selected, in
   * order to save
   * memory and file loading time for unused AutoRoutines.
   *
   * <p>
   * The generators are only run when the DriverStation is disabled and the
   * alliance is known.
   *
   * <p>
   * One way to keep this clean is to make an `Autos` class that all of your
   * subsystems/resources
   * are <a href="https://en.wikipedia.org/wiki/Dependency_injection">dependency
   * injected</a> into.
   * Then create methods inside that class that take an {@link AutoFactory} and
   * return an {@link
   * AutoRoutine}.
   *
   * <h3>Example:</h3>
   ...
   *
   * <p>
   * This is done to load autonomous commands when and only when they are
  ...
  /**
   * Gets a Command that schedules the selected auto routine. This Command shares
   * the lifetime of
   * the scheduled Command. This Command can directly be bound to a trigger, like
   * so:
   *
   * <pre>
   * <code>
   ...
   *
   * <p>
   * If you plan on using this {@link Command} in a {@code Trigger} it is
   * recommended to use
